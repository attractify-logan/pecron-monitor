...
- Split monitor alert delivery, rules automation, shutdown-restore policy, and shared telemetry helpers into focused modules while keeping `PecronMonitor` behavior and method contracts unchanged (#66).
...
- Split Home Assistant telemetry formatting, last-known-state handling, deferred-port updates, and energy-counter publishing into `ha_state.py`, leaving `ha_bridge.py` focused on MQTT connection, retry, and command dispatch (#66).
