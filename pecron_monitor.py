...
def _truthy(v):
    """Normalize Pecron boolean-like values to True/False."""
    if v is None:
        return False
...
def _get_first_present(kv: dict, paths):
    """Return the first non-None value present in kv following any of the provided tuple paths."""
    for p in paths:
        cur = kv
        ok = True
        for key in p:
